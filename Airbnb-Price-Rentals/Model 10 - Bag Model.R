...
bag10 <- randomForest(price ~ accommodates + room_type + bathrooms + 
                        review_scores_location + bedrooms + 
                        neighbourhood_cleansed + cleaning_fee + 
                        cancellation_policy + number_of_reviews + extra_people + 
                        review_scores_value + review_scores_cleanliness + 
                        security_deposit + neighbourhood_group_cleansed +
                        minimum_nights + review_scores_rating + 
                        review_scores_checkin + days_since_last_review + 
                        review_scores_accuracy + guests_included +
                        maximum_nights + beds + host_acceptance_rate + 
                        instant_bookable + host_is_superhost + is_location_exact,
                      data = train_data_cleaning,
                      mtry = ncol(train_data_cleaning)-1,
                      ntree=1000)

# Predict the price on test_data_cleaning
pred_price10_test <- predict(bag10, newdata = test_data_cleaning)
RMSE(pred_price10_test, test_data_cleaning$price)
...
###############################################################################
# MODEL 10 TO SCORING DATA
#########################

pred_price10_scoring <- predict(bag10, newdata = scoring_data_cleaning)
...
submissionDataFrame = data.frame(id = scoring_data_cleaning$id, price = pred_price10_scoring)
