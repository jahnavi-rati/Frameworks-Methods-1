...
tuneGrid <- expand.grid(n.trees = 750,
                        interaction.depth = c(1,2,3,4,5),
                        shrinkage = (1:100)*0.001,
                        n.minobsinnode=c(5,10,15))

garbage <- capture.output(cvModel8 <- train(price ~ accommodates + room_type + 
                                              ...
                                              review_scores_checkin + 
                                              days_since_last_review + 
                                              review_scores_accuracy + guests_included + 
                                              maximum_nights + beds + host_acceptance_rate + 
...
set.seed(617)
cvBoost8 <- gbm(price ~ accommodates + room_type + bathrooms + review_scores_location +
                  bedrooms + neighbourhood_cleansed + cleaning_fee + cancellation_policy +
                  number_of_reviews + extra_people + review_scores_value + 
                  review_scores_cleanliness + security_deposit + neighbourhood_group_cleansed +
                  minimum_nights + review_scores_rating + review_scores_checkin + 
                  days_since_last_review + review_scores_accuracy + guests_included +
                  maximum_nights + beds + host_acceptance_rate + instant_bookable + 
                  host_is_superhost + is_location_exact,
                data = train_data_cleaning,
                distribution = "gaussian",
                n.trees = cvModel8$bestTune$n.trees,
                interaction.depth = cvModel8$bestTune$interaction.depth,
                shrinkage = cvModel8$bestTune$shrinkage,
                n.minobsinnode = cvModel8$bestTune$n.minobsinnode)


# Predict the price on test_data_cleaning
pred_price8_test <- predict(cvBoost8, newdata = test_data_cleaning, n.trees = 750)
RMSE(pred_price8_test, test_data_cleaning$price) # if it shows an error, then uncomment next line and run it


#sqrt(mean((pred_price8_test - test_data_cleaning$price)^2))
...
###############################################################################
# MODEL 8 TO SCORING DATA
#########################

pred_price8_scoring <- predict(cvBoost8, newdata = scoring_data_cleaning)
...
write.csv(submissionDataFrame, 'my_sixth_submission.csv', row.names = F)
getwd()
