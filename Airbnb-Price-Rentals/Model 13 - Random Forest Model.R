...
set.seed(617)
forest13 <- randomForest(price ~ accommodates + room_type + bathrooms + 
...
pred_price13_test <- predict(forest13, newdata = test_data_cleaning)
RMSE(pred_price13_test, test_data_cleaning$price)
...
###############################################################################
# MODEL 11 TO SCORING DATA
#########################

pred_price13_scoring <- predict(forest13, newdata = scoring_data_cleaning)
...
submissionDataFrame = data.frame(id = scoring_data_cleaning$id, price = pred_price13_scoring)
write.csv(submissionDataFrame, 'my_seventh_submission.csv', row.names = F)
getwd()
