...
df_with_num_variables <- data_cleaning[, num_variables[c(-1, -11)]]
...
corMatrix <- as.data.frame(cor(df_with_num_variables[, -c(1, 11)]))
...
library(vtreat); library(xgboost); library(caret)


# Remove price column from the train and test sets
train_data1 <- train_data_cleaning %>%
  select(-id, -price, -last_review)

test_data1 <- test_data_cleaning %>%
  select(-id, -price, -last_review)


# Creating a treatment and new input variables
trt <- designTreatmentsZ(dframe = train_data1,
                         varlist = names(train_data1))
new_variables <- trt$scoreFrame[trt$scoreFrame$code%in% c('clean','lev'),
                                'varName']


# Setting up the data frame for the train and test variables
train_input <- prepare(treatmentplan = trt,
                       dframe = train_data1,
                       varRestriction = new_variables)
...
# Running the rounds of treatment
set.seed(617)
tune_nrounds <- xgb.cv(data = as.matrix(train_input),
                       label = train_data_cleaning$price,
                       nrounds=500,
                       nfold = 5,
                       verbose = 0)


# Plotting the performance of rounds
...
# Predict the price on test_input
pred_price7_test <- predict(xgboost2, newdata = as.matrix(test_input))
RMSE(pred_price7_test, test_data_cleaning$price) # if it shows an error, then uncomment next line and run it
...
###############################################################################
# MODEL 7 TO SCORING DATA
#########################


# Remove price column from the train and test sets
scoring_factor <- scoring_data_cleaning %>%
  select(-id, -last_review)



# Creating a treatment and new input variables
trt_scoring <- designTreatmentsZ(dframe = scoring_factor,
                                 varlist = names(scoring_factor))
new_variables_scoring <- trt_scoring$scoreFrame[trt_scoring$scoreFrame$code%in% c('clean','lev'),
                                                'varName']


# Setting up the data frame for scoring
scoring_input <- prepare(treatmentplan = trt_scoring,
                         dframe = scoring_factor,
                         varRestriction = new_variables_scoring)

pred_price7_scoring <- predict(xgboost2, newdata = as.matrix(scoring_input))
...
submissionDataFrame = data.frame(id = scoring_data_cleaning$id, price = pred_price7_scoring)
write.csv(submissionDataFrame, 'my_fourth_submission.csv', row.names = F)
getwd()
