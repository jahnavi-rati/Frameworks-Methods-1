...
train_data_factor <- train_data_cleaning %>%
  mutate_if(is.character, as.factor)

test_data_factor <- test_data_cleaning %>%
  mutate_if(is.character, as.factor)


set.seed(617)
boost9 <- gbm(price ~ accommodates + room_type + bathrooms + review_scores_location +
...
pred_price9_test <- predict.gbm(boost9, newdata = test_data_cleaning, n.trees = 750)
RMSE(pred_price9_test, test_data_cleaning$price) # if it shows an error, then uncomment next line and run it
...
###############################################################################
# MODEL 9 TO SCORING DATA
#########################

pred_price9_scoring <- predict.gbm(boost9, newdata = scoring_data_cleaning)
...
submissionDataFrame = data.frame(id = scoring_data_cleaning$id, price = pred_price9_scoring)
write.csv(submissionDataFrame, 'my_fifth_submission.csv', row.names = F)
getwd()
