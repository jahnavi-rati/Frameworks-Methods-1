...
cvModel6 <- train(price ~ accommodates + room_type + bathrooms + review_scores_location + 
                  ...
                  method = "rf", ntree = 750, trControl = trControl, 
                  tuneGrid = tuneGrid)

cvModel6

pred_price6_test <- predict(cvModel6, newdata = test_data_cleaning)
RMSE(pred_price6_test, test_data_cleaning$price)

#


###############################################################################
# MODEL 6 TO SCORING DATA
#########################

pred_price6_scoring <- predict(cvModel6, newdata = scoring_data_cleaning)
...
submissionDataFrame = data.frame(id = scoring_data_cleaning$id, price = pred_price6_scoring)
